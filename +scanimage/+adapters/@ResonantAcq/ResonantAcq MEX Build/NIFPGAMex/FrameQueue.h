...
class FrameQueue : public AbstractConsumerQueue {
 ...
 public:
  ...
  // If the input parameter force is set to true, the frame will be forced into
  // the queue, even if the frame queue is full. It does this by replacing the
  // last frame in the queue with the control frame.
  // 
  // Return value is true if push was successful, false otherwise.
  bool push_back(const void *src, bool force);
