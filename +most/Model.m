classdef Model < most.DClass
    ...
    properties (Abstract, Hidden, SetAccess=protected)
        ...
        mdlInitSetExcludeProps; %String cel array of props to forcible exclude from initialization set method call.
    end
        ...
        function initialize(obj)
            ...
            try
                for ii=1:length(propNames)
                    ...
                    if ~isempty(mp.SetMethod) && strcmpi(mp.SetAccess,'public') && ~ismember(propNames{ii},obj.mdlInitSetExcludeProps)
    ...
    methods (Hidden) % Ultimately, protected
        ...
        function [origPropSet, anyPropNotFound] = mdlApplyPropSet(obj,propSet,tfOrder)
            ...
            try
                ...
                for c = 1:numel(propNames)
                    ...
                    try
                        ...
                        if ~isprop(obj,pname)
                            warning('Model:propNotFound', 'Property ''%s'' not found. Ignoring.',pname);
                            anyPropNotFound = true;
                        else                       
                            if (strcmp(pname,'beamLengthConstants') | strcmp(pname, 'beamPzAdjust') | strcmp(pname, 'beamPowers'))
                                %disp('No beam is configured')
                            else
        ...
        % MAT-file.
        function mdlSavePropSet(obj,propSet,fname,cullExtraProps)
            assert(isstruct(propSet));
            assert(ischar(fname));
            ...
            % overwritten
            % right now cullextraprops flag does not work. unused
            % properties will be removed even if append flag is on due to
            % the nested structure
            if exist(fname,'file')==2 && ~cullExtraProps
        ...
        function mdlSavePropSetFromList(obj,propList,fname,varargin)
            propSet = obj.mdlGetPropSet(propList);
            obj.mdlSavePropSet(propSet,fname,varargin{:});            
        end
...
%                 end
    ...
    methods (Static,Access=protected)
        ...
        function propNames = getDefaultConfigProps(clsName)            
            fcn      = @(x) (...
                    strcmpi(x.SetAccess,'public') ...
                    && strcmpi(x.GetAccess,'public') ...
                    && ~x.Transient && ~x.Constant && ~x.Hidden && ~x.Dependent);         
            propNames = most.Model.getAllPropsWithCriterion(clsName,fcn);
        end
        
        function propNames = getDefaultHeaderProps(clsName)
            fcn = @(x)(strcmpi(x.GetAccess,'public') && ~x.Hidden && ~x.Transient);
            propNames = most.Model.getAllPropsWithCriterion(clsName,fcn);
        end
